...
$(function () {
    if (typeof (Storage) !== "undefined") {
        ...
        iziToast.warning({
            title: 'AVISO',
            message: "Desculpe, mas seu navegador não suporta armazenamento local, portanto, este aplicativo não funcionará como deveria. Tente atualizar seu navegador primeiro.",
            overlay: true,
            zindex: 999,
            position: 'center',
            timeout: 20000,
...
$("#days td.inactive").on("click", function () {
    iziToast.error({
        title: 'ERRO',
        message: "Você só pode marcar consultas de hoje em diante.",
        overlay: true,
        zindex: 999,
        position: 'center',
        timeout: 3000,
...
function make_appointment() {
    if (is_empty() == false) {
        ...
        if (is_overlap() == false) {
            ...
            iziToast.error({
                title: 'ERRO',
                message: "Esta consulta está sobrecarregando outra.",
                overlay: true,
                zindex: 999,
                position: 'center',
                timeout: 3000,
...
function compare() {
    ...
    if (startTime == endTime) {
        ...
        iziToast.warning({
            title: 'AVISO',
            message: "O horário de início não deve ser igual ao horário de término",
            overlay: true,
            zindex: 999,
            position: 'center',
            timeout: 2000,
...
function clear_storage(){
    ...
    iziToast.success({
        title: 'SUCESSO',
        message: 'Todas as consultas foram deletadas.',
    });
}
...
$("#submit").on("click", function () {
    if ($(this).attr("data-editing") === "true") {
        // Se estivermos editando, chame a função update_appointment
        update_appointment();
    } else {
        // Caso contrário, crie um novo compromisso
        make_appointment();
    }
});
...
function delete_appointment(id){
    ...
    if (data[0] !== null) {
        ...
        iziToast.success({
            title: 'SUCESSO',
            message: 'A consulta foi deletada.',
        });
